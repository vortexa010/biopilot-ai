...
html, body, p, label, input, textarea, h1, h2, h3, h4, h5, h6,
.stMarkdown, .stTextInput, .stTextArea, .stSelectbox, .stButton {
...
with st.sidebar:
    ...
    if stored_key:
        ...
        st.warning("Demo API key is not configured.")
        with st.expander("🔐 Developer setup", expanded=False):
            ...
            st.caption("For deployment, add GROQ_API_KEY in Streamlit Cloud Secrets.")
    ...
    with st.expander("ℹ️ About", expanded=False):
        ...
        st.write("It transforms a gene and disease context into hypotheses, validation plans, and a publication-style research report.")
    st.caption("Microsoft Agents League 2026 · GitHub Copilot-assisted · Powered by Groq")
...
if should_run:
    if not api_key:
        st.markdown("<div class='error-note'>⚠️ Demo AI connection is not configured. Add GROQ_API_KEY in Streamlit Secrets or Developer setup.</div>", unsafe_allow_html=True)
        st.stop()
    if not gene:
...
if st.session_state.last_results:
    ...
    <div class="metric-grid">
      ...
      <div class="metric-card"><div class="metric-label">Publication Potential</div><div class="metric-value">{safe_score(gene + focus_area, 85, 10)}/100</div></div>
    </div>
    <div class="info-box">
